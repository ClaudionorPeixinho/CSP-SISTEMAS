...
export default function Admin() {
  ...
  const [settingsForm, setSettingsForm] = useState({ whatsappNumber: "", contactEmail: "" });
  const { data: apps = [], isLoading, refetch } = trpc.applications.listAll.useQuery(undefined, { enabled: Boolean(user?.role === "admin") });
  const { data: settingsData } = trpc.settings.get.useQuery(undefined, { enabled: Boolean(user?.role === "admin") });
  ...
  const saveSettings = trpc.settings.update.useMutation({ onSuccess: () => { toast.success("Contato do administrador atualizado"); utils.settings.get.invalidate(); }, onError: (e) => toast.error("Não foi possível salvar", { description: friendlyErrorMessage(e) }) });
  ...
  useEffect(() => { if (settingsData) setSettingsForm({ whatsappNumber: settingsData.whatsappNumber ?? "", contactEmail: settingsData.contactEmail ?? "" }); }, [settingsData]);
  const saveContactSettings = (event: React.FormEvent) => {
    event.preventDefault();
    const problems: string[] = [];
    if (settingsForm.contactEmail && !/^\S+@\S+\.\S+$/.test(settingsForm.contactEmail)) problems.push("E-mail de contato inválido");
    if (settingsForm.whatsappNumber && settingsForm.whatsappNumber.replace(/\D/g, "").length < 8) problems.push("Número de WhatsApp inválido (mínimo 8 dígitos)");
    if (problems.length > 0) { toast.error("Revise os campos", { description: problems.join(" • ") }); return; }
    saveSettings.mutate({ whatsappNumber: settingsForm.whatsappNumber.trim(), contactEmail: settingsForm.contactEmail.trim() });
  };
  ...
  return <div className="min-h-screen bg-[#f8fafc] text-slate-950"><header className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-xl"><div className="container flex h-[76px] items-center justify-between"><Brand /><div className="flex items-center gap-4"><span className="hidden text-right sm:block"><strong className="block text-xs font-black">{user.name || "Administrador"}</strong><small className="text-[10px] font-bold uppercase tracking-[0.1em] text-emerald-600">acesso admin</small></span><button onClick={logout} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 text-slate-500 hover:border-red-200 hover:text-red-500" aria-label="Sair"><LogOut className="h-4 w-4" /></button></div></div></header><main className="container py-10"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><div className="mb-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-600"><LayoutDashboard className="h-3.5 w-3.5" /> painel de controle</div><h1 className="text-4xl font-black tracking-[-0.06em]">Aplicativos<span className="text-emerald-500">.</span></h1><p className="mt-2 text-sm text-slate-500">Cadastre, organize e destaque as soluções da sua vitrine.</p></div><div className="flex gap-3"><Link href="/" className="inline-flex h-11 items-center justify-center rounded-full border border-slate-200 bg-white px-4 text-xs font-black uppercase tracking-[0.11em] text-slate-600"><ArrowLeft className="mr-2 h-4 w-4" /> ver site</Link><button onClick={() => { setForm(emptyForm); setEditingId(null); setShowForm(!showForm); }} className="inline-flex h-11 items-center justify-center rounded-full bg-slate-950 px-5 text-xs font-black uppercase tracking-[0.11em] text-white hover:bg-emerald-600">{showForm ? <X className="mr-2 h-4 w-4" /> : <Plus className="mr-2 h-4 w-4" />}{showForm ? "fechar" : "adicionar aplicativo"}</button></div></div>
      <div className="mt-8 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><h2 className="text-xl font-black">Contato do administrador</h2><p className="mt-1 text-sm text-slate-500">Número de WhatsApp e e-mail que vão receber as mensagens enviadas pelo formulário "Vamos conversar" do site.</p><form onSubmit={saveContactSettings} className="mt-6 grid gap-5 sm:grid-cols-2"><Field label="WhatsApp" type="tel" value={settingsForm.whatsappNumber} onChange={(value) => setSettingsForm({ ...settingsForm, whatsappNumber: value })} /><Field label="E-mail" type="email" value={settingsForm.contactEmail} onChange={(value) => setSettingsForm({ ...settingsForm, contactEmail: value })} /><div className="flex justify-end sm:col-span-2"><button disabled={saveSettings.isPending} className="inline-flex h-11 items-center justify-center rounded-full bg-emerald-500 px-6 text-xs font-black uppercase tracking-[0.1em] text-slate-950 hover:bg-emerald-400 disabled:opacity-50"><Save className="mr-2 h-4 w-4" />{saveSettings.isPending ? "salvando..." : "salvar contato"}</button></div></form></div>
